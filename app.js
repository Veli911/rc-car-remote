...
let tiltAutoCenterStartedAt = 0;
let tiltAutoCenterSinSum = 0;
let tiltAutoCenterCosSum = 0;
let tiltAutoCenterSampleCount = 0;

const TILT_FULL_SCALE_DEG = 40;
const TILT_AUTOCENTER_SETTLE_MS = 180;
const TILT_AUTOCENTER_WINDOW_MS = 500;
const TILT_AUTOCENTER_MIN_SAMPLES = 8;
  ...
  modeButtons.forEach((button) => {
    button.addEventListener('click', () => {
      ...
      if (currentMode === 'tilt') {
        ...
        tiltAutoCenterStartedAt = performance.now();
        tiltAutoCenterSinSum = 0;
        tiltAutoCenterCosSum = 0;
        tiltAutoCenterSampleCount = 0;
        resetSteering();
...
function onTiltOrientation(event) {
  ...
  // When Tilt is enabled, device-orientation can emit a few transient samples
  // before the sensor frame settles. Calibrating from the very first event can
  // make the next tiny movement look like a huge angle jump. Keep steering at
  // zero briefly, then use the circular mean of several stable samples as the
  // automatic center. This gives the same result as pressing Center manually,
  // without requiring the user to press it every time.
  if (tiltNeedsAutoCenter) {
    const elapsed = performance.now() - tiltAutoCenterStartedAt;
    ...
    if (elapsed < TILT_AUTOCENTER_SETTLE_MS) {
      return;
    }

    const radians = absoluteAngle * Math.PI / 180;
    tiltAutoCenterSinSum += Math.sin(radians);
    tiltAutoCenterCosSum += Math.cos(radians);
    tiltAutoCenterSampleCount += 1;

    if (elapsed < TILT_AUTOCENTER_WINDOW_MS ||
        tiltAutoCenterSampleCount < TILT_AUTOCENTER_MIN_SAMPLES) {
      return;
    }

    tiltCenterOffsetDeg = Math.atan2(
      tiltAutoCenterSinSum / tiltAutoCenterSampleCount,
      tiltAutoCenterCosSum / tiltAutoCenterSampleCount
    ) * 180 / Math.PI;
